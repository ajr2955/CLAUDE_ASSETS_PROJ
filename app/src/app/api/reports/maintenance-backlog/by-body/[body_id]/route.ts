...
// GET /api/reports/maintenance-backlog/by-body/:body_id (operations_manager+)
// Returns backlog for a specific responsible body: open work orders grouped by
// priority, status, and category, with overdue counts and cost totals.
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ body_id: string }> }
) {
  const auth = requireAuth(req, "operations_manager");
  if (auth instanceof NextResponse) return auth;

  const { body_id } = await params;
  ...
  if (!body) return err("ResponsibleBody not found", 404);

  const now = new Date();

  const where = {
    assignedToBodyId: body_id,
  ...
  return ok({
    body: { id: body.id, name: body.name, body_type: body.bodyType },
    total_open: total,
