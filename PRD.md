...
**Acceptance Criteria:**
- [x] Asset detail page includes an "Events" tab
- [x] Tab renders a chronological list of events, newest first; each item shows: event type, category badge (business/operational/governance), occurred_at, recorded by, responsible body, description, and document count
- [x] Governance events auto-generated by the system shown with a robot icon
- [x] Events with `warnings_overridden: true` in metadata shown with a yellow alert badge and the justification text
- [x] "Add Event" button (for `department_user`+) opens a form: event_type (grouped select by category), occurred_at, description, attach document (optional), responsible_body
- [x] Infinite scroll or "Load more" for long logs
- [x] Filter events by category (business / operational / governance) and date range
- [x] Typecheck passes
- [x] Verify changes work in browser
